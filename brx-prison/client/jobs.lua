...
function CreateJobBlip(noItem) -- Used globally
    ...
    -- Define blip settings per job
    local blipSettings = {
        ['sparky'] = { sprite = 402, color = 1, name = "Electric Work" },
        ['mario'] = { sprite = 402, color = 1, name = "Plumbing Work" },
        ['muk'] = { sprite = 402, color = 1, name = "Maintenance Work" }
    }

    local jobBlip = blipSettings[currentJob] or { sprite = 402, color = 1, name = "Prison Job" } -- Default

    SetBlipSprite(currentBlip, jobBlip.sprite)
    SetBlipDisplay(currentBlip, 4)
    SetBlipScale(currentBlip, 0.8)
    SetBlipAsShortRange(currentBlip, true)
    SetBlipColour(currentBlip, jobBlip.color)

    BeginTextCommandSetBlipName('STRING')
    AddTextComponentSubstringPlayerName(jobBlip.name)
    EndTextCommandSetBlipName(currentBlip)

    if noItem then return end
    TriggerServerEvent('brx-prison:server:CheckChance')
end
...
local function StartWork()
    ...
    QBCore.Functions.Progressbar('work', Lang:t('info.working'), math.random(5000, 10000), false, true, {
                    ...
                    options = {
                        {
                            ...
                            canInteract = function()
                                return inJail 
                                    and currentJob == k -- ✅ Restrict to assigned job only
                                    and not Config.Locations.jobs[k][i].done 
                                    and not isWorking 
                                    and i == currentLocation
                            end,
                            action = function()
                                StartWork()
                            end
